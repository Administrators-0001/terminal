...
void Implementation::SetDefaultScopeOnechar()
{
    _wantsAnsiInputScope = true;
}
...
{
    if (_wantsAnsiInputScope && IsEqualGUID(rguidAttribute, GUID_PROP_INPUTSCOPE))
    {
...
HRESULT Implementation::AnsiInputScope::GetInputScopes(InputScope** pprgInputScopes, UINT* pcCount) noexcept
{
    ...
    // I have no idea if it should be IS_ONECHAR or something else like IS_ALPHANUMERIC_HALFWIDTH.
    // The documentation is non-existent and the implementation is a cancerous mass of madness.
    scopes[0] = IS_ONECHAR;
